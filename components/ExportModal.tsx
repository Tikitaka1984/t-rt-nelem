...
interface ExportModalProps {
  content: Article | { topic: string; text: string } | { topic: string, events: TimelineEvent[] };
  type: 'article' | 'essay' | 'timeline';
  onClose: () => void;
}
...
export const ExportModal: React.FC<ExportModalProps> = ({ content, type, onClose }) => {
  ...
  const getExportText = () => {
    switch(type) {
      ...
      case 'essay': {
        const essay = content as { topic: string, text: string };
        return `Téma: ${essay.topic}\n\n${essay.text}`;
      }
      case 'timeline': {
